...
class InitiativeTracker {
    ...
    createComponentsRow(rowTurn, rowButtonAdd, selectPlayerMenu) {
        ...
        if (this.selectPlayerMenu.stringSelectMenu.options.length < 1 || this.selectPlayerMenu.stringSelectMenu.options.length > 25) {
            this.rowSelect = null; // Set to null if the condition is not met
        }
    ...
    async updateUI() {
        const turnOrder = await calculateTurnOrder(this.players, this.currentTurn, this.turnNumber, true, refreshPlayerData);
